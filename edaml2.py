...
# graph_insight(test)
# plt.show()
subset = ['date', 'date_block_num', 'shop_id', 'item_id','item_cnt_day','ID']
...
# count_price = train.ID.value_counts().sort_index(ascending=False)
# plt.subplot(224)
# count_price.hist(figsize=(20,5))
# plt.xlabel('ID');
# plt.title('Original Distiribution')
...
# d = dict(zip(apply_df.columns[7:],list(np.array(list(apply_df.columns[7:])) - 1)))
